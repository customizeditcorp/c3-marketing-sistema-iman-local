#!/usr/bin/env python3
"""
Regenerar foto de Carlos con camisa blanca y logo C! bordado
...
def main():
    print("=" * 80)
    print("🎬 REGENERACIÓN DE CARLOS - CAMISA BLANCA CON LOGO C!")
    ...
    print("\n" + "=" * 80)
    print("CARLOS - CAMISA BLANCA + LOGO C! BORDADO")
    print("=" * 80)
    
    carlos_photo = TEAM_PHOTOS_DIR / "carlos-cordero.jpg"
    
    # Prompt optimizado (< 1000 caracteres)
    carlos_prompt = """Using @carlos as base: PRESERVE @carlos face, expression, arms-crossed pose EXACTLY (100% Nano Banana). CHANGE: Replace gray shirt with crisp white button-down shirt. ADD: Embroidered "C!" logo on left chest (same size and style as Luis's coral shirt logo - small, professional). KEEP: Facebook lanyard, watch unchanged. BACKGROUND: Professional office with horizontal aluminum blinds, off-white wall, natural lighting from left, soft focus. Precise edges, no artifacts. High resolution, warm professional tone. DO NOT modify @carlos face or pose."""
    
    success = generate_image_with_reference(
        prompt=carlos_prompt,
        reference_image_path=carlos_photo,
        reference_tag="carlos",
        output_filename="01_carlos_white_shirt_c_logo.png"
    )
    
    if success:
        print("\n✅ Imagen de Carlos con camisa blanca generada exitosamente")
    else:
    ...
    print("   ✓ Camisa cambió de gris a blanca")
    print("   ✓ Logo C! bordado visible en pecho izquierdo")
    print("   ✓ Tamaño del logo similar al de Luis")
